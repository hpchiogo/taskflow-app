...
//pour la creation de projet des utilisateur
...
export async function deleteProjectById(projectId: string) {
  try {
    ...
    console.log(`Projet avec l'ID ${projectId} suprimé avec succès.`)
  } catch (error) {
    console.error(error)
    throw new Error()
  }
...
export async function getProjectUsers(idProject: string) {
  try {
    ...
    const users =
      projectWithUsers?.users.map((projectUser) => projectUser) || []
    return users
  } catch (error) {
    console.error(error)
    throw new Error()
