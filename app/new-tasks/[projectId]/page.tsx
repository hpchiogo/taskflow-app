...
const Page = ({ params }: { params: Promise<{ projectId: string }> }) => {
  ...
  const fetchInfos = async (projectId: string) => {
    try {
      ...
      // on extrait juste les vrais "User"
      setUsersProject(associatedUsers.map((item) => item.user))
    } catch (error) {
    ...
    <Wrapper>
      <div>
        ...
        <div className='flex flex-col md:flex-row md:justify-between'>
          ...
          <div className='md:w-3/4 md:mt-0  md:ml-4'>
            ...
            <div className='flex justify-end' onClick={handleSubmit}>
              <button className='btn mt-4 btn-md btn-primary'>Creer la tache</button>
            </div>
          </div>
        </div>
